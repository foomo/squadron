...
installs the squadron or given units

```
squadron up [SQUADRON] [UNIT...] [flags]
```

### Examples

```
  squadron up storefinder frontend backend --namespace demo --build --push -- --dry-run
```
...
```
  ...
  -n, --namespace string         set the namespace name or template (default, squadron-{{.Squadron}}-{{.Unit}}) (default "default")
      ...
      --push                     pushes units to the registry
